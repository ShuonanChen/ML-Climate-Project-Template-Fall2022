...
+ diag_matrix(rep_vector(square(sigma), Nsample));
	...
	matrix ker_gp1gp2(real[] x1, real[] x2, real sdgp1, real sdgp2, real lscale1, real lscale2, real sigma) { 
		...
		for (n in 1:size(x1)) {
			cov[n, n] = cov[n, n] + square(sigma) +   square(1e-8);
		}
		return cholesky_decompose(cov);
	}
...
generated quantities{
	...
	f_grid = gp1gp2_pred_rng(x_grid[,1], x_grid[,2], log_yn, x[,1], x[,2], alpha[1], alpha[2], rho[1], rho[2], sigma, 1e-10)*logysd + logymean;
